...
for i = 1:1
    ...
    nbStruct = fitcnb(train.X,train.Y);
    
    classZeroIndex = ismember(nbStruct.ClassNames,0);
    estimates = nbStruct.DistributionParameters{classZeroIndex,1}
   
    [predictedLabels,score,cost]= predict(nbStruct, valid.X);
...
Classification = cell({nbStruct.ModelParameters.Method});
...
title('ROC for Classification by NiveBase on Training Set')
...
[predictedTestLabels,testScore,testCost]= predict(nbStruct, newTest.X);
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC for Classification by NiveBase on TestData')
